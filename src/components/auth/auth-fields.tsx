...
const inputClass =
  "w-full rounded-[8px] border border-line bg-(--bg-2) px-3 py-2.5 text-sm text-ink outline-none transition-[border-color,box-shadow] duration-150 placeholder:text-ink-3 focus:border-(--accent) focus:shadow-[0_0_0_3px_color-mix(in_oklch,var(--accent)_22%,transparent)]";
...
export function SubmitButton({
  ...
  return (
    <button
      type="submit"
      disabled={pending}
      className="mt-1 cursor-pointer rounded-[8px] bg-(--accent) px-4 py-2.75 text-sm font-semibold text-white transition-[filter,opacity] duration-150 hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60"
    >
